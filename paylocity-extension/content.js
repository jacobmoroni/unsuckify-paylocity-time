(function () {
    ...
    function buildUI(state) {
        ...
        function makeCodeRow(ci, code) {
            ...
            const labelCell = h('td', { style: labelTdStyle },
                h('span', { style: `font-size:12px;color:${code.unknown ? C.orange : C.text};vertical-align:middle;` }, baseLabel(code.label)),
        ...
        // Rebuild the grid rows from the current saved codes, preserving any
        // values the user has already typed in.
        function rebuildGrid() {
            // Snapshot typed values: { codeKey: { di: hours } }
            const snapshot = {};
            ...
            // Rebuild codeList from current favorites (non-fav rows are dropped on rebuild)
            codeList.length = 0;
            favoriteJobCodes().forEach(jc => codeList.push({ label: jc.label, value: jc.value, payTypeId: jc.payTypeId || '-1', unknown: false }));
            // Re-add any non-fav extras that were already in the grid
            const nonFavExtras = [];
            sortedDays.forEach(({ entries }) => {
                Object.values(entries).forEach(entry => {
                    if (entry.LaborLevel && parseFloat(entry.Duration) > 0) {
            ...
            // Replace tbody rows (keep totalRow)
    ...
    // ── Quick entry modal (triggered by 4 PM alarm) ───────────────────
        ...
        const rows = codes.map(jc => {
            ...
            return {
                ...
                el: h('div', { style: 'display:flex;align-items:center;gap:8px;padding:4px 0;' },
                    h('span', { style: `font-size:13px;color:${C.text};flex:1;` },
                        baseLabel(jc.label),
    ...
    function init() {
        ...
        // Direct message from popup (tab already open and focused).
        chrome.runtime.onMessage.addListener((msg) => {
            if (msg.type === 'pcty-show-reminder') showReminderIfNeeded(!!msg.force);
        });

        // Storage flag: covers new tabs, post-redirect, and alarm triggers.
        chrome.storage.onChanged.addListener((changes, area) => {
            if (area !== 'local' || !changes.pendingReminder?.newValue) return;
            chrome.storage.local.get('pendingReminderForce', (data) => {
                chrome.storage.local.remove(['pendingReminder', 'pendingReminderForce']);
                showReminderIfNeeded(!!data.pendingReminderForce);
            });
        });

        // Fallback: flag was set before this content script loaded.
        chrome.storage.local.get(['pendingReminder', 'pendingReminderForce'], (data) => {
            if (data.pendingReminder) {
                chrome.storage.local.remove(['pendingReminder', 'pendingReminderForce']);
                showReminderIfNeeded(!!data.pendingReminderForce);
            }
        });
    ...
    // force=true  → always show modal (manual button / notification click)
    // force=false → skip if today already has hours (alarm path)
    function showReminderIfNeeded(force = false) {
        const today = new Date().toISOString().split('T')[0];

        if (!location.href.includes('/Timesheet')) {
            sessionStorage.setItem('pcty_pending_reminder', '1');
            if (force) sessionStorage.setItem('pcty_pending_reminder_force', '1');
            location.href = 'https://webtime2.paylocity.com/WebTime/Employee/Timesheet';
            return;
        }
        ...
        const poll = setInterval(() => {
            ...
            if (state && Object.keys(state.days).length > 0) {
                clearInterval(poll);
                if (force) {
                    showQuickEntryModal(state, today);
                } else {
                    const alreadyFilled = Object.values(state.days)
                        .filter(d => d.date === today)
                        .some(d => Object.values(d.entries).some(e => parseFloat(e.Duration) > 0));
                    if (!alreadyFilled) showQuickEntryModal(state, today);
                }
            } else if (++attempts >= 30) {
                clearInterval(poll);
            }
    ...
    function boot() {
        init();
        if (sessionStorage.getItem('pcty_pending_reminder')) {
            const force = !!sessionStorage.getItem('pcty_pending_reminder_force');
            sessionStorage.removeItem('pcty_pending_reminder');
            sessionStorage.removeItem('pcty_pending_reminder_force');
            showReminderIfNeeded(force);
        }
    }
